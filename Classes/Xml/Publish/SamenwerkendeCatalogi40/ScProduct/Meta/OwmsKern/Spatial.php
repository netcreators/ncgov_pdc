...
class Spatial extends Node
{
    ...
    {
        $this->setTagName('dcterms:spatial')
            ->addAttribute('scheme', $parameter->getSpatialScheme())
            ->addAttribute('resourceIdentifier', $parameter->getSpatialResourceIdentifier())
            ->setContent(
                htmlspecialchars(
                    $parameter->getSpatial(),
                    ENT_QUOTES
                )
            );
    }
