...
class Authority extends Node
{
    ...
    {
        $this->setTagName('overheid:authority')
            ->addAttribute('scheme', $parameter->getAuthorityScheme())
            ->addAttribute('resourceIdentifier', $parameter->getAuthorityResourceIdentifier())
            ->setContent(
                htmlspecialchars(
                    $parameter->getAuthority(),
                    ENT_QUOTES
                )
            );
    }
