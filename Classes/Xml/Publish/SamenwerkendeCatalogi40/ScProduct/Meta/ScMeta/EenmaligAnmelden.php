 ...
 * @subpackage Xml
 */
class EenmaligAnmelden extends Node
{
    ...
    public function __construct(Product $product)
    {
        ...
        $reverseEnumerationMapping = array(
            Product::REQUEST_ONLINE_SINGLE_SIGN_ON_NO => 'nee',
            Product::REQUEST_ONLINE_SINGLE_SIGN_ON_YES => 'ja'
        );

        $this->setTagName('overheidproduct:eenmaligAanmelden')
            ->setContent(
                htmlspecialchars(
                    $reverseEnumerationMapping[$product->getScmetaRequestOnlineSingleSignOn()],
                    ENT_QUOTES
                )
            );
    }
