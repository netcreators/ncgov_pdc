...
class OwmsMantel extends Node
{
    ...
    public function __construct(
        ...
        $this->setTagName('overheidproduct:owmsmantel');

        foreach ($product->getAudience() as $audience) {
            $this->addChild(
                $parameter->getObjectManager()->get(
                    OwmsMantel\Audience::class,
                    $audience
                )
            );
        }

        foreach ($product->getKeywords() as $keyword) {
            $this->addChild(
                $parameter->getObjectManager()->get(
                    OwmsMantel\Subject::class,
                    $keyword
                )
            );
        }
        ...
        $this->addChild(
            $parameter->getObjectManager()->get(
                OwmsMantel\AbstractSummary::class,
                $product
            )
        );
    }
